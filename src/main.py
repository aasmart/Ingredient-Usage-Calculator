...
def get_data(file_name) -> pd.DataFrame:
    """Takes a CSV file name and reads in the data as a dataframe"""

    return pd.read_csv(file_name + ".csv")

def write_data(data: pd.DataFrame, out_file):
    """
    Takes a dataframe and the name of a CSV file and writes the
    data to the file
    """
...
def calculate_grams_per_dollar(
    ...
    "Calculates the weight per unit of cost"
...
def main():
    ...
    FileSelector(frame, "Products CSV:").grid(column = 0, row = 0)
    
    ttk.Button(frame, text="Import").grid(column=0, row = 1)
